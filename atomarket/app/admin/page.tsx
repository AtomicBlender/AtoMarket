  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-5xl space-y-4 px-4 py-8">
        <h1 className="text-2xl font-semibold text-slate-100">Admin Dispute Queue</h1>
        {rows.map(({ dispute, market }) => (
          <article key={dispute.id} className="space-y-3 rounded-xl border border-slate-800 bg-slate-900/70 p-4">
            <h2 className="text-lg font-semibold text-slate-100">{market?.title ?? dispute.market_id}</h2>
            <p className="text-sm text-slate-300">
              Proposal: {dispute.proposed_outcome} | Challenge deadline: {formatDateTime(dispute.challenge_deadline)}
            </p>
        ...
        {rows.length === 0 ? (
          <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-5 text-slate-400">No challenged disputes.</div>
        ) : null}
      </section>
    </main>
  );
