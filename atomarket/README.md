...
3. Set `CRON_SECRET` and configure your Vercel cron to send `Authorization: Bearer <CRON_SECRET>` for `/api/resolve`.
4. Apply migration: `supabase/migrations/20260305093000_atomarket_mvp.sql`.
5. Optionally seed sample markets with `supabase/seed.sql`.
6. Set at least one admin account in `profiles` (`is_admin = true`).
7. Install and run:
   - `npm install`
   - `npm run dev`
