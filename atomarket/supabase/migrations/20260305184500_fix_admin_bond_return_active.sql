  ...
  if found then
    if v_proposal.status = 'CHALLENGED' then
      ...
      if p_outcome = v_proposal.proposed_outcome then
        -- Proposal upheld: proposer gets proposer bond back + challenger's forfeited bond.
        ...
        -- Proposal overturned: challenger gets challenge bond back + proposer's forfeited bond.
        update public.profiles
        ...
        -- Defensive fallback: return both bonds and reject proposal.
        update public.profiles
      ...
      -- Unchallenged proposal resolved by admin: return proposer bond and mark status.
      update public.profiles
      set neutron_balance = neutron_balance + v_proposal.bond_neutrons
      where id = v_proposal.proposed_by;
...
begin
  ...
  if found then
    -- In invalidation, return proposer bond.
    update public.profiles
    set neutron_balance = neutron_balance + v_proposal.bond_neutrons
    where id = v_proposal.proposed_by;
